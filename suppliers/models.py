...
class Supplier(models.Model):
    ...
    address_line_1 = models.CharField("Endereço", max_length=255, blank=True)
    address_line_2 = models.CharField("Complemento", max_length=255, blank=True)
    city = models.CharField("Cidade", max_length=100, blank=True)
    state = models.CharField("Estado", max_length=50, blank=True)
    zip_code = models.CharField("CEP", max_length=10, blank=True)
    minimum_order_value = models.DecimalField("Valor Mínimo para Pedido", max_digits=10, decimal_places=2, validators=[MinValueValidator(0)], null=True, blank=True)
    order_response_deadline = models.TimeField("Prazo de Resposta para Pedidos", null=True, blank=True)
    departments = models.ManyToManyField('products.Departamento', related_name='suppliers')
